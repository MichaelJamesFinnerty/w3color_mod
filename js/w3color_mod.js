...
/* modifications to toColorObject() and colorObject() functions 
by Michael James Finnerty at Mountain Oak Design*/
...
function toColorObject(c) {
  var x, y, typ, arr = [], arrlength, i, opacity, match, a, hue, sat, rgb, colornames = [], colorhexs = [], ncolRe; //added ncolRe variable
  c = w3trim(c.toLowerCase());
  x = c.substr(0,1).toUpperCase();
  y = c.substr(1);
  a = 1;
    
  //ncolRe is regEx that looks for five 
  //consecutive number characters
  ncolRe = /\w{5,}/; 
    
  //the new !(ncolRe.exec(y) will bounce 
  //out hex colors that start with B or C 
  //and then continue with numbers 
  //(e.g., B22222 - FireBrick and 
  //C71585 - MediumVioletRed))    
  if ((x == "R" || x == "Y" || x == "G" || x == "C" || x == "B" || x == "M" || x == "W") && !isNaN(y) && !(ncolRe.exec(y))) {c = "ncol(" + c + ")";} 
...
window.getColorArr = getColorArr;
window.toColorObject = toColorObject;
